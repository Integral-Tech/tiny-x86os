...
static uint8_t PADDING[] = {[0] = 0x80, [1 ... 63] = 0x00};
 ...
 */
void md5_update(md5_context_t *ctx, uint8_t *input_buf, size_t input_len) {
  uint32_t input[16];
  unsigned int offset = ctx->size % 64;
  ctx->size += (uint64_t)input_len;

  // Copy each byte in input_buf into the next space in our context input
...
void md5_string(char *input, uint8_t *result) {
  ...
  memcpy(result, ctx.digest, MD5SUM_LEN);
}
...
void md5_file(FILE *file, uint8_t *result) {
  ...
  while ((input_size = fread(input_buf, 1, INPUT_BUF_SIZE, file)) > 0)
    md5_update(&ctx, (uint8_t *)input_buf, input_size);
